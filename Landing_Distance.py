...
def getLanding_Wingloading(Wingloading):
    ...
    rho = (isa_model(0,0)[2])
    s_50 = Wingloading(1 / (2 * 9.81 * constants.epsilon_L)) * (constants.v_50 * constants.v_50_min - constants.v_L * constants.v_L) + constants.h_50 / constants.epsilon_L

    s_R = -Wingloading*(1.13^2) / (rho * constants.c_Lmac_Ldg * constants.b_M)
